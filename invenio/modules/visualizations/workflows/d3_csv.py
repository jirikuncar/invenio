...
class ColumnInlineForm(WebDepositForm):
    ...
    type = fields.SelectField(
        choices=[('string', 'String'), ('integer', 'Number')],
        default='string',
        widget=ColumnInput(widget=widgets.Select(),
                           class_="col-xs-4 col-pad-0"),
        widget_classes='form-control',
    )


class D3CsvForm(VisualizationForm):
    """D3 CSV visualization form"""
    #
    # Fields
    #
    ...
    columns = fields.DynamicFieldList(
        ...
        validators=[validators.Required(), list_length(
            min_num=1, element_filter=filter_empty_helper(['id']),
        )],
    )
    ...
    # Form configuration
    #
    _title = _('New D3 CSV View')

    @property
    def groups(self):
        return super(D3CsvForm, self).groups + [
...
# Workflow
#
class d3_csv(visualization_base):
    """D3 csv visualization"""

    name = "D3 CSV View"
    name_plural = "D3 CSV Views"
    draft_definitions = {
        'default': D3CsvForm,
    }
